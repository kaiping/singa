 ...
 * \return the shape construction
 */
MSHADOW_XINLINE Shape<4> Shape4( index_t s3, index_t s2, index_t s1,
                                 index_t s0 ) {
  Shape<4> s;
...
struct Tensor: public expr::ContainerExp<Tensor<Device, dimension>> {
 ...
 public:
  ...
  MSHADOW_XINLINE Tensor(real_t *dptr,
                         const Shape<dimension> &shape): dptr((real_t *)dptr), shape(shape) {}
  /*!
   ...
   */
  MSHADOW_XINLINE Tensor<Device, dimension> Slice(index_t begin,
      index_t end) const {
 ...
 public:
  ...
  /*!\brief functions to fit expression template */
  template<typename E>
  inline Tensor<Device, dimension> &operator=( const
      expr::Exp<E, expr::type::kMapper> &exp ) {
    return this->__assign( exp );
  }
  /*!\brief functions to fit expression template */
  template<typename E>
  inline Tensor<Device, dimension> &operator=( const
      expr::Exp<E, expr::type::kComplex> &exp ) {
    return this->__assign( exp );
  }
};
...
struct Tensor<Device, 1>: public expr::ContainerExp<Tensor<Device, 1>> {
 ...
 public:
  ...
  MSHADOW_XINLINE Tensor(real_t *dptr, Shape<1> shape) : dptr(dptr),
    shape(shape) {}
  ...
  MSHADOW_XINLINE real_t &operator[](index_t idx) {
    return dptr[ idx ];
  }
  MSHADOW_XINLINE const real_t &operator[](index_t idx)const {
    return dptr[ idx ];
  }
 public:
  ...
  template<typename E>
  inline Tensor<Device, 1> &operator=( const expr::Exp<E, expr::type::kMapper>
                                       &exp ) {
    return this->__assign( exp );
  }
  template<typename E>
  inline Tensor<Device, 1> &operator=( const expr::Exp<E, expr::type::kComplex>
                                       &exp ) {
    return this->__assign( exp );
  }
};
...
inline Tensor<Device, dim> NewTensor(const Shape<dim> &shape, real_t initv,
...
inline void MapReduceKeepLowest( Tensor<cpu, 1> dst,
                                 const expr::Exp<E, etype> &exp, real_t scale = 1.0f );
/*! \brief refer to comment of cpu ver \sa MapReduceKeepLowest */
template<typename Saver, typename Reducer, typename E, int etype>
inline void MapReduceKeepLowest( Tensor<gpu, 1> dst,
                                 const expr::Exp<E, etype> &exp, real_t scale = 1.0f );
...
inline void MapReduceKeepHighDim( Tensor<cpu, 1> dst,
                                  const expr::Exp<E, etype> &exp, real_t scale = 1.0f );
/*! \brief refer to comment of cpu ver \sa MapReduceKeepHighDim */
template<typename Saver, typename Reducer, typename E, int etype>
inline void MapReduceKeepHighDim( Tensor<gpu, 1> dst,
                                  const expr::Exp<E, etype> &exp, real_t scale = 1.0f );
