...
inline DotExp<TA, TB, false, false> dot( const ContainerExp<TA> &lhs,
    const ContainerExp<TB> &rhs ) {
  return DotExp<TA, TB, false, false>( lhs.self(), rhs.self(), 1.0f );
}
/*! \brief dot operator def */
template<typename TA, typename TB>
inline DotExp<TA, TB, true, false> dot( const TransposeExp<TA> &lhs,
                                        const ContainerExp<TB> &rhs ) {
  return DotExp<TA, TB, true, false>( lhs.exp, rhs.self(), 1.0f );
}
/*! \brief dot operator def */
template<typename TA, typename TB>
inline DotExp<TA, TB, false, true> dot( const ContainerExp<TA> &lhs,
                                        const TransposeExp<TB> &rhs ) {
  return DotExp<TA, TB, false, true>( lhs.self(), rhs.exp, 1.0f );
}
/*! \brief dot operator def */
template<typename TA, typename TB>
inline DotExp<TA, TB, true, true> dot( const TransposeExp<TA> &lhs,
                                       const TransposeExp<TB> &rhs ) {
  return DotExp<TA, TB, true, true>( lhs.exp, rhs.exp, 1.0f );
}
/*! \brief dot operator def */
template<typename TA, typename TB, bool ltrans, bool rtrans>
inline DotExp<TA, TB, ltrans, rtrans> operator*( const
    DotExp<TA, TB, ltrans, rtrans> &lhs, real_t rhs ) {
  return DotExp<TA, TB, ltrans, rtrans>( lhs.lhs_, lhs.rhs_, lhs.scale_ * rhs );
}
/*! \brief scale of dot operation */
template<typename TA, typename TB, bool ltrans, bool rtrans>
inline DotExp<TA, TB, ltrans, rtrans> operator*( real_t lhs,
    const DotExp<TA, TB, ltrans, rtrans> &rhs ) {
...
inline BinaryMapExp < OP, TA, TB,
       (ta | tb | type::kMapper) > MakeExp( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
  return BinaryMapExp < OP, TA, TB, (ta | tb | type::kMapper) > ( lhs.self(),
         rhs.self() );
}
...
template<typename OP, typename TA, typename TB, int ta, int tb>
inline BinaryMapExp < OP, TA, TB,
       (ta | tb | type::kMapper) > F( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<OP>( lhs, rhs );
}
/*! \brief operator overload for const */
template<typename OP, typename TA, int ta>
inline BinaryMapExp < OP, TA, ScalarExp,
(ta | type::kMapper) > F( const Exp<TA, ta> &lhs, const ScalarExp &rhs ) {
  return MakeExp<OP>( lhs, rhs );
}
/*! \brief operator overload for const */
template<typename OP, typename TB, int tb>
inline BinaryMapExp < OP, ScalarExp, TB,
(tb | type::kMapper) > F( const ScalarExp &lhs, const Exp<TB, tb> &rhs ) {
  return MakeExp<OP>( lhs, rhs );
}
...
inline BinaryMapExp < op::plus, TA, TB,
       (ta | tb | type::kMapper) > operator+( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<op::plus>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TA, typename TB, int ta, int tb>
inline BinaryMapExp < op::minus, TA, TB,
       (ta | tb | type::kMapper) > operator-( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
...
/*! \brief operator overload */
template<typename TA, typename TB, int ta, int tb>
inline BinaryMapExp < op::mul, TA, TB,
       (ta | tb | type::kMapper) > operator*( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
...
/*! \brief operator overload */
template<typename TA, typename TB, int ta, int tb>
inline BinaryMapExp < op::div, TA, TB,
       (ta | tb | type::kMapper) > operator/( const Exp<TA, ta> &lhs,
const Exp<TB, tb> &rhs ) {
...
inline BinaryMapExp < op::plus, TA, ScalarExp,
       (ta | type::kMapper) > operator+( const Exp<TA, ta> &lhs,
const ScalarExp &rhs ) {
  return MakeExp<op::plus>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TA, int ta>
inline BinaryMapExp < op::minus, TA, ScalarExp,
       (ta | type::kMapper) > operator-( const Exp<TA, ta> &lhs,
const ScalarExp &rhs ) {
  return MakeExp<op::minus>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TA, int ta>
inline BinaryMapExp < op::mul, TA, ScalarExp,
       (ta | type::kMapper) > operator*( const Exp<TA, ta> &lhs,
const ScalarExp &rhs ) {
  return MakeExp<op::mul>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TA, int ta>
inline BinaryMapExp < op::div, TA, ScalarExp,
       (ta | type::kMapper) > operator/( const Exp<TA, ta> &lhs,
const ScalarExp &rhs ) {
  return MakeExp<op::div>( lhs, rhs );
}
...
inline BinaryMapExp < op::plus, ScalarExp, TB,
       (tb | type::kMapper) > operator+( const ScalarExp &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<op::plus>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TB, int tb>
inline BinaryMapExp < op::minus, ScalarExp, TB,
       (tb | type::kMapper) > operator-( const ScalarExp &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<op::minus>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TB, int tb>
inline BinaryMapExp < op::mul, ScalarExp, TB,
       (tb | type::kMapper) > operator*( const ScalarExp &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<op::mul>( lhs, rhs );
}
/*! \brief operator overload */
template<typename TB, int tb>
inline BinaryMapExp < op::div, ScalarExp, TB,
       (tb | type::kMapper) > operator/( const ScalarExp &lhs,
const Exp<TB, tb> &rhs ) {
  return MakeExp<op::div>( lhs, rhs );
}
...
/*! \brief make expression */
template<typename OP, typename TA, int ta>
inline UnaryMapExp < OP, TA,
(ta | type::kMapper) > MakeExp( const Exp<TA, ta> &src ) {
...
template<typename OP, typename TA, int ta>
inline UnaryMapExp < OP, TA,
(ta | type::kMapper) > F( const Exp<TA, ta> &src ) {
