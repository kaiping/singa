...
class Random<cpu> {
 ...
 private:
  ...
  inline real_t RandNext2( void ) {
    return (static_cast<real_t>( rand() ) + 1.0 ) / (static_cast<real_t>
           (RAND_MAX) + 2.0);
  }
  /*!
...
class Random<gpu> {
 public:
  ...
  Random<gpu>(int seed) {
    ...
    utils::Assert(status == CURAND_STATUS_SUCCESS,
   ...
   */
  template<int dim>
  inline void SampleUniform(Tensor<gpu, dim> &dst, real_t a = 0.0f,
                            real_t b = 1.0f) {
    if ( a == 0.0f && b == 1.0f ) {
   ...
   */
  template<int dim>
  inline void SampleGaussian(Tensor<gpu, dim> &dst, real_t mu = 0.0f,
                             real_t sigma = 1.0f) {
   ...
   */
  template<int dim>
  inline expr::ReshapeExp<Tensor<gpu, 1>, dim, 1> gaussian( Shape<dim> shape,
      real_t mu = 0.0f, real_t sigma = 1.0f) {
...
#else
    status = curandGenerateNormalDouble(gen_, buffer_.dptr, buffer_.shape[0], mu,
                                        sigma);
#endif
