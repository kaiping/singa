...
def create_shard(hostfile, localdir, workers, resize, mfile):
    '''
    run mpi job to parse images and insert records to shard.dat
    mfile: mean file path
    '''
    cmd='mpirun -np %d -hostfile %s ./loader --dir=%s --mean=%s --width=%d --height=%d ' %(len(workers), hostfile, localdir, mfile, resize, resize)
    print cmd
    os.system(cmd)
    sys.exit()
...
if __name__=="__main__":
    ...
    parser.add_argument('--local', default='/data1/wangwei/lapis/validation', help='local shard dir, abs path')
    parser.add_argument('--hdfs', default='imagenet12/validation', help='hdfs dir')
    args=parser.parse_args();
    if not os.path.isabs(args.local):
        print 'local folder %s is not a abs path' % args.local
    ...
    partition(records, workers, args.m, args.local, args.r)
    ...
    download(args.hdfs, args.local, workers, exedir)

    if 'loader' not in os.listdir(os.getcwd()):
        print 'loader exec cannot be found in the current working directory'
    else:
        create_shard(args.host, args.local, workers, args.s, args.mfile)
