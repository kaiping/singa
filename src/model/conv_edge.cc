...
void ConvEdge::Setup(bool set_param) {
  ...
  TShape3& shape = (bottom_->feature(this)).shape;
  num_ = shape[3];
  channels_ = shape[2];
  height_ = shape[1];
  width_ = shape[0];
...
void ConvEdge::Forward(const Blob4 &src, Blob4 *dest, bool overwrite) {
  ...
  for (int n = 0; n < num_; n++) {
    im2col(src[n].dptr, channels_, height_, width_, ksize_, pad_, stride_,
           col_fea_.dptr);
  ...
  for (int n = 0; n < num_; n++) {
    ...
    for (int g = 0; g < num_groups_; g++) {
      ...
      if (dest_grad != nullptr)
        col_grad_.Slice(sk, ek) = dot(weight.Slice(sm, em).T(), src_grad2.Slice(sm,
                                      em));
    }
